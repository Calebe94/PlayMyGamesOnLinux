...
DESCRIPTION
	...
	completely rewrites San Andreas controls handling and ditches DirectInput in favour of XInput. This
	...
	* TWO different control mappings - one setup matching PS2 selectable setups and second one
	...
	* An INI file with tons of options - including toggleable vibration, selecting controls setup,
	  "Invert Look" option from console versions and more!
	...
	* Full co-op support - the game can make full use of two pads or bind the first pad to second player, so co-op
	  can be played even if only one pad is connected
